// --- START OF FILE export.js - PARTE 1/3 ---
...
function InventorySystem() {
    ...
    InventorySystem.Export = function() {
        ...
        function captureHighQualityChart(canvas) {
            ...
            try {
                // 🚀 MEJORA 1: Scale 4K (era 2, ahora 4)
                const scale = 4; // Factor de escala para calidad 4K
                ...
                if (chartInstance) {
                    ...
                    chartInstance.options.animation = false; // Sin animación para captura
                    
                    // 🚀 MEJORA 4: Optimización de fonts y elementos
                    if (chartInstance.options.plugins) {
                        ...
                        if (chartInstance.options.plugins.title) {
                            chartInstance.options.plugins.title.font = {
                                ...chartInstance.options.plugins.title.font,
                                size: (chartInstance.options.plugins.title.font?.size || 16) * 1.2
                            };
                        }
                ...
                // 🚀 MEJORA 5: Exportar en máxima calidad PNG
        ...
        // 🎨 FUNCIÓN AUXILIAR: Generar títulos inteligentes
        function generateSmartChartTitle(canvas, index) {
            // Intentar obtener título del Chart.js
            const chartInstance = Chart.getChart(canvas.id);
            ...
            if (chartContainer) {
                const titleEl = chartContainer.querySelector('h3, .chart-title, h2');
                if (titleEl?.textContent) {
                    return titleEl.textContent.trim();
                }
            }
            
            // Generar título basado en el ID del canvas
            const canvasId = canvas.id || '';
            if (canvasId.includes('distribution')) return 'Distribución de Inventario';
            if (canvasId.includes('warehouse')) return 'Análisis por Almacén';
            if (canvasId.includes('negative')) return 'Productos con Inventario Negativo';
            if (canvasId.includes('pallet')) return 'Material en Pedido por Almacén';
            if (canvasId.includes('top')) return 'Productos Principales';
            if (canvasId.includes('trend')) return 'Tendencias de Inventario';
            
            // Título por defecto mejorado
            return `Análisis Visual ${index + 1}`;
        }

        // 🎨 FUNCIÓN AUXILIAR: Metadata de gráficos
        function addChartMetadata(doc, canvas, x, y, width, colors) {
            doc.setFontSize(7);
            doc.setFont(undefined, 'normal');
            doc.setTextColor(...colors.dark);
            
            const now = new Date();
            const metadata = `Generado: ${now.toLocaleDateString('es-ES')} | Resolución: 4K | Fuente: Sistema de Inventario`;
            
            // Centrar metadata
            doc.text(metadata, x + width/2, y, { align: 'center' });
        }

        // CONTINÚA EN PARTE 2/3...
// --- PARTE 2/3: CONTINUACIÓN DE export.js ---

        // 🚀 NUEVA FUNCIÓN: Layout inteligente de gráficos 
        function addSmartChartsGrid(doc, charts, startY, pageWidth, pageHeight, margin, colors) {
            if (!charts || charts.length === 0) {
                doc.setFontSize(12);
                doc.text('No hay gráficos disponibles en este momento.', margin, startY + 20);
                return startY + 40;
            }
            
            let currentY = startY;
            
            // 🚀 ALGORITMO: Decidir layout según número de gráficos
            const chartCount = charts.length;
            let layout, chartWidth, chartHeight;
            
            if (chartCount === 1) {
                // Un solo gráfico: Usar todo el ancho
                ...
                chartWidth = (pageWidth - (2 * margin) - 20) / 2;
                chartHeight = 100;
            } else if (chartCount <= 4) {
                // 3-4 gráficos: Grid 2x2
                layout = { cols: 2, rows: 2 };
                chartWidth = (pageWidth - (2 * margin) - 20) / 2;
                chartHeight = 80;
            } else {
                // 5+ gráficos: Grid 2x3 con paginación
            ...
            charts.forEach((canvas, index) => {
                ...
                const gridX = (gridIndex % layout.cols) * (chartWidth + 20);
                const gridY = Math.floor(gridIndex / layout.cols) * (chartHeight + 30);
                
                const xPos = margin + gridX;
                const yPos = currentY + gridY;
                
                // 🚀 MEJORA: Títulos inteligentes y descriptivos
                let chartTitle = generateSmartChartTitle(canvas, index);
                ...
                try {
                    if (canvas.width > 0 && canvas.height > 0) {
                        ...
                        if (imageData) {
                            ...
                            // Añadir metadata del gráfico
                            addChartMetadata(doc, canvas, xPos, yPos + chartHeight + 12, chartWidth, colors);
                        }
                    } else {
                        doc.setFontSize(9);
            ...
            const totalRows = Math.ceil(Math.min(charts.length, 6) / layout.cols);
            return currentY + (totalRows * (chartHeight + 30)) + 20;
        }
        ...
        function generateReportPDF(reportType, category, options = {}) {
            ...
            // Variables para el dashboard mejorado
            ...
            // 📄 PÁGINA 1: Portada y Resumen Ejecutivo para Dashboard
            ...
            // 📈 FUNCIÓN MEJORADA: Sección de gráficos con layout inteligente
            function addEnhancedChartsSection() {
                ...
                // 🚀 USAR NUEVA FUNCIÓN DE GRID INTELIGENTE
                const finalY = addSmartChartsGrid(doc, charts, 35, pageWidth, pageHeight, margin, colors);
                
                // Añadir resumen de análisis visual
                if (charts.length > 0) {
                    doc.setFontSize(10);
                    doc.setTextColor(...colors.dark);
            ...
            function addChartsSection(sectionTitle, startY) {
                ...
                for (let i = 0; i < charts.length; i++) {
                    ...
                    let chartTitle = generateSmartChartTitle(canvas, i);
            ...
            if (reportType === 'complete') {
                ...
                switch (category) {
                    ...
                    case 'all':
                        ...
                        break;
                    case 'dashboard':
                        // 🚀 Generar dashboard mejorado con gráficos 4K
                        addCoverPage();
...
// --- PARTE 3/3: CONTINUACIÓN DE export.js ---
        ...
        function generatePrintHTML(reportType, category, options = {}) {
            ...
            const styles = `
                <style>
                    ...
                    .chart-item h3 { font// --- CONTINUACIÓN PARTE 3/3: generatePrintHTML y resto de funciones ---
            ...
            if (reportType === 'tab') {
                ...
                if (category === 'dashboard') {
                    ...
                    charts.forEach((canvas, index) => {
                        ...
                        try {
                            if (canvas.width > 0 && canvas.height > 0) {
                                const imageData = captureHighQualityChart(canvas) || canvas.toDataURL('image/png', 0.95);
        ...
        function formatInsightsPDF(doc, insights, startY, margin, pageWidth) {
            ...
            insights.forEach((insight, index) => {
                ...
                switch (insight.type) {
                    case 'success':
                        backgroundColor = [229, 255, 229];
                        textColor = [34, 139, 34];
                        break;
                    case 'warning':
                        backgroundColor = [255, 248, 220];
                        textColor = [255, 140, 0];
                        break;
                    case 'danger':
                        backgroundColor = [255, 235, 235];
                        textColor = [220, 20, 60];
                        break;
                    default: // info
        ...
        function createExportModalSingleton() {
            ...
            function populateDynamicOptions(category, isCompleteReportMode) {
                ...
                container.innerHTML = '<p style="padding: 20px; text-align: center; color: #666;">Opciones adicionales disponibles según la categoría seleccionada.</p>';
            }
        ...
        // 🚀 NUEVO moduleAPI con funciones 4K
        moduleAPI = {
            init,
            exportToPDF,
            exportToExcel,
            ...
            formatInsightsPDF,
            formatInsightsHTML,
            // 🚀 FUNCIONES DE GRÁFICOS 4K
            captureHighQualityChart,     // Exponer para uso en HTML
            addSmartChartsGrid,          // Para uso interno avanzado
            generateSmartChartTitle,     // Para títulos inteligentes
            addChartMetadata            // Para metadata de gráficos
        };
